...
function carregarProdutos() {
    ...
    database.ref('produtos').on('value', (snapshot) => {
        ...
        Object.keys(dados).forEach(id => {
            ...
            // Tratamento da URL: pega a primeira se houver várias separadas por vírgula
            const primeiraMidia = p.imagem ? p.imagem.split(',')[0].trim() : '';
            
            // AGORA SIM: Chamamos a função que decide se é vídeo ou imagem
            const htmlMidia = renderizarMidia(primeiraMidia);
            ...
            grid.innerHTML += `
                <div class="card-padrao ${isBloqueado ? 'bloqueado' : ''}">
                    ...
                    <div class="conteudo-card">
                        ...
                        <button onclick="alternarStatus('${id}', '${p.status}')" class="btn-secundario" style="width:100%; margin-top:10px;">
                            <i class="fas ${isBloqueado ? 'fa-eye' : 'fa-eye-slash'}"></i> 
                            ${isBloqueado ? ' Ativar' : ' Bloquear'}
                        </button>
                    </div>
                </div>`;
        });
...
// Mantenha apenas uma versão desta função:
...
function renderizarMidia(url) {
    if (!url) return `<img src="https://via.placeholder.com/300x200" class="img-card">`;

    // Verifica se a URL contém extensões de vídeo comuns
    const ehVideo = url.toLowerCase().includes('.mp4') || 
    ...
    if (ehVideo) {
        return `
            <video src="${url}" 
                   ...
                   loop 
                   playsinline 
                   style="width: 100%; object-fit: cover; height: 200px; display: block;">
            </video>`;
    } else {
        return `<img src="${url}" class="img-card" style="width: 100%; object-fit: cover; height: 200px;">`;
    }
}
